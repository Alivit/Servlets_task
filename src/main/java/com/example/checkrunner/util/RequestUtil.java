package com.example.checkrunner.util;
...
import com.example.checkrunner.exception.CardOrProductException;
...
public class RequestUtil {
    ...
    public void comparison(){
        ...
        for(Map.Entry<Integer,Integer> item : orderMap.entrySet()){
            products.forEach(product ->{
                if (product.getId() == item.getKey()){
                    priceCalculation(product.getId() - 1, item);
                }
            });
        }
    ...
    private void priceCalculation(int i, Map.Entry<Integer, Integer> item){
        ...
        double newPrice = products.get(i).getPrice() * item.getValue();
        if(products.get(i).getStatus() != null || item.getValue() >= 5) {
            newPrice = newPrice - percent(products.get(i).getPrice() * item.getValue(), 10);
        }
