package com.example.checkrunner.logic;
    ...
    private static void printProduct(RequestUtil request){
        for (int i = 0; i < request.getPromotional().size(); i++) {
            checkList.add(String.format("%-17s %-18s %-10s %-10s",
                    request.getPromotional().get(i).getProduct().getName(),
                    request.getPromotional().get(i).getProduct().getPrice(),
                    request.getPromotional().get(i).getProduct().getAmount(),
