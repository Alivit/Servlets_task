package com.example.checkrunner.file.impls;
...
public class ReceiptPDF implements Inputable {
     ...
     */
    private final static File FILE_RECEIPT = new File("src/main/webapp/resources/Receipt.pdf");
    ...
    private static void addProduct(Document document, RequestUtil request) throws DocumentException {
        ...
        try {
            for (int i = 0; i < request.getPromotional().size(); i++) {
                table.addCell(new PdfPCell(new Phrase(request.getPromotional().get(i).getProduct().getName())));
                table.addCell(new PdfPCell(new Phrase(String.valueOf(request.getPromotional().get(i).getProduct().getPrice()))));
                table.addCell(new PdfPCell(new Phrase(String.valueOf(request.getPromotional().get(i).getProduct().getAmount()))));
